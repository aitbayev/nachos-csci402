...
void Lock::Acquire() {
	...
	if (status == FREE){ //if lock is available
		...
		//cout<<currentThread->getName()<<" acquired lock \n";
	}
	else{
		waitQueue->Append((void *)currentThread);
		currentThread->Sleep();
		//cout<<currentThread->getName()<<" Trying to acquire, in a line for a lock, sleeping \n";
	}
...
void Lock::Release() {
	...
	if (!waitQueue->IsEmpty()){
		...
		//cout<<thread->getName()<<" retrieved from wait list, got the lock\n";
		lockOwner = thread; //Make it lock owner
		scheduler->ReadyToRun(thread); //wakeup the thread
	} else{
		//cout<<currentThread->getName()<<" released lock \n";
		status = FREE; //lock is available again
...
void Condition::Wait(Lock* conditionLock) { //ASSERT(FALSE); 
...

	{
		...
		cout << "waiting lock does not equal condition lock \n";  //??
...
void Condition::Broadcast(Lock* conditionLock) { 
	...
	if (conditionLock != waitingLock){
		...
		cout << "Error: condition lock is not equal waiting lock"; //??
